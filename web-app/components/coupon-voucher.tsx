    ...
    <div className="relative w-full">
      ...
      <div className="relative bg-gradient-to-br from-white via-cyan-50 to-purple-50 rounded-l-2xl overflow-hidden shadow-2xl border-2 border-cyan-300/50 flex flex-row w-full max-w-full">
        ...
        <div className="flex-1 min-w-[350px] w-full p-4 flex flex-col justify-between relative h-[200px]">
          ...
          {/* Target Store/Item Info */}
          {(couponDesign.targetStore || couponDesign.targetItem) && (
            <div className="mb-2 p-2 bg-purple-500/10 border border-purple-400/30 rounded">
              {couponDesign.targetStore && (
                <div className="flex items-center gap-1.5">
                  <div className="w-1 h-1 bg-purple-400 rounded-full"></div>
                  <span className="text-[10px] text-slate-600">Store: <span className="font-semibold text-slate-800">{couponDesign.targetStore}</span></span>
                </div>
              )}
              {couponDesign.targetItem && (
                <div className="flex items-center gap-1.5">
                  <div className="w-1 h-1 bg-purple-400 rounded-full"></div>
                  <span className="text-[10px] text-slate-600">Item: <span className="font-semibold text-slate-800">{couponDesign.targetItem}</span></span>
                </div>
              )}
            </div>
          )}
