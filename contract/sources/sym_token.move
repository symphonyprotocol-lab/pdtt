module pdtt::sym_token {
    ...
    const ASSET_SYMBOL: vector<u8> = b"SYM";
    ...
    entry fun init_module(admin: &signer) {
        ...
        primary_fungible_store::create_primary_store_enabled_fungible_asset(
            constructor_ref,
            option::none(),
            utf8(b"Symphony Coin"), /* name */
            utf8(ASSET_SYMBOL), /* symbol */
            8, /* decimals */
            utf8(
                b"https://tan-necessary-stingray-268.mypinata.cloud/ipfs/bafkreidoofcnzxmmsuckasxqmg7oju3xdx6pwgmbuswe5iebgcx3cvxtni"
            ), /* icon */
            utf8(b"https://symphonyprotocol.com/") /* project */
        );
    ...
    public fun get_metadata(): Object<Metadata> {
        let asset_address = object::create_object_address(&@pdtt, ASSET_SYMBOL);
        object::address_to_object<Metadata>(asset_address)
    }

    /// Deposit function override to ensure that the account is not denylisted and the sym_token is not paused.
    /// OPTIONAL
    ...
    /// Withdraw function override to ensure that the account is not denylisted and the sym_token is not paused.
    /// OPTIONAL
    ...
    public entry fun set_pause(pauser: &signer, paused: bool) acquires State {
        let asset = get_metadata();
        assert!(
            object::is_owner(asset, signer::address_of(pauser)),
            error::permission_denied(ENOT_OWNER)
        );
        let state =
            borrow_global_mut<State>(
                object::create_object_address(&@pdtt, ASSET_SYMBOL)
            );
    ...
    fun assert_not_paused() acquires State {
        let state =
            borrow_global<State>(object::create_object_address(&@pdtt, ASSET_SYMBOL));
    ...
    #[test(creator = @pdtt)]
    fun test_basic_flow(creator: &signer) acquires ManagedFungibleAsset, State {
        init_module(creator);
    ...
    #[test(creator = @pdtt, aaron = @0xface)]
    ...
    fun test_paused(creator: &signer) acquires ManagedFungibleAsset, State {
        init_module(creator);
        let creator_address = signer::address_of(creator);
        mint(creator, creator_address, 100);
